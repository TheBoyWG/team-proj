(function($) {
    ...
    function offsetSearch() {
        ...
        if ($(window).width() >= 1200) {
            ...
            box_search_popup.offset({
                left: col_size_search.offset().left,
                top: col_size_search.offset().top
            });
        } else if ($(window).width() < 1200) {
            box_search_popup.width("90%");
            box_search_popup.offset({
                left: $("body").width() * 0.035,
                top: col_size_search.offset().top
            });
        }
    }
    ...
    $(window).on('load', function() {
        ...
        $("#account-setting").tabs({
            "active": getUrlParameter('active_tab')
        });
